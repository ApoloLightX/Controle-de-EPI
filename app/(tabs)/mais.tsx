...
export default function MoreScreen(){const{session,logout}=useApp();const isAdmin=session?.role==='admin';return <Screen safeTop><Header title="Mais" subtitle={isAdmin?'Ferramentas administrativas e configurações.':'Acesso pessoal e configurações.'}/><DemoBanner/><Card style={{padding:0,overflow:'hidden'}}>{rows.filter(r=>!r.admin||isAdmin).map((r,i)=><Pressable accessibilityRole="button" accessibilityLabel={`Abrir ${r.label}`} key={r.label} onPress={()=>router.push(r.route as never)} style={[styles.row,i>0&&styles.border]}><View style={styles.icon}><Ionicons name={r.icon} size={20} color={colors.blue}/></View><Text style={styles.label}>{r.label}</Text><Ionicons name="chevron-forward" size={18} color={colors.muted}/></Pressable>)}</Card><Pressable accessibilityRole="button" accessibilityLabel="Sair da conta" onPress={async()=>{await logout();router.replace('/login')}} style={styles.logout}><Ionicons name="log-out-outline" size={20} color={colors.red}/><Text style={styles.logoutText}>Sair da conta</Text></Pressable></Screen>}
