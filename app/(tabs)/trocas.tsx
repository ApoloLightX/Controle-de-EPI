...
export default function SwapsScreen() {
  ...
  return <Screen safeTop>
    <Header title="Trocas" subtitle={isAdmin ? `${pending} em análise • ${waiting} aguardando estoque` : 'Acompanhe suas solicitações de troca.'} right={<Pressable accessibilityRole="button" accessibilityLabel="Nova solicitação de troca" onPress={() => router.push('/swap/new')} style={styles.add}><Ionicons name="add" size={17} color={colors.blue} /><Text style={styles.addText}>Nova</Text></Pressable>} />

    <View style={styles.filters}>{filters.map(item => <Pressable accessibilityRole="radio" accessibilityState={{ checked: filter === item }} key={item} onPress={() => setFilter(item)} style={[styles.filter, filter === item && styles.filterActive]}><Text style={[styles.filterText, filter === item && styles.filterTextActive]}>{item}</Text></Pressable>)}</View>
    ...
    {swaps.length ? swaps.map(s => {
      ...
      return <Card key={s.id} style={{ gap: 10 }}>
        ...
        {s.photoUri ? <View style={styles.photoWrap}><Image accessibilityLabel={`Foto anexada à troca de ${epi?.name ?? 'EPI'}`} source={{ uri: s.photoUri }} style={styles.photo} resizeMode="cover" /><View style={styles.tag}><Ionicons name="image" size={15} color={colors.blue} /><Text style={styles.tagText}>Foto anexada</Text></View></View> : null}
...
const styles = StyleSheet.create({
  ...
  photoWrap: { gap: 7 }, photo: { width: '100%', height: 180, borderRadius: 12, backgroundColor: colors.border }, tag: { alignSelf: 'flex-start', flexDirection: 'row', gap: 5, alignItems: 'center', backgroundColor: '#EAF1FF', borderRadius: 999, paddingHorizontal: 9, paddingVertical: 6 }, tagText: { color: colors.blue, fontWeight: '800', fontSize: 11 },
