...
import { Card, EmptyState, Header, Screen, SearchField, SecondaryButton, StatusBadge } from '@/components/UI';
...
export default function PeopleScreen() {
  ...
  if (!isAdmin) {
    ...
    return <Screen safeTop>
      ...
      </Card> : <EmptyState title="Perfil indisponível" body="Entre novamente para atualizar a sessão." />}
    </Screen>;
  }

  return <Screen safeTop>
    <Header title="Pessoas" subtitle={`${data.employees.length} colaboradores cadastrados`} right={<Pressable accessibilityRole="button" accessibilityLabel="Cadastrar colaborador" onPress={() => router.push('/employee-form')} style={styles.add}><Ionicons name="add" size={24} color="#fff"/></Pressable>} />
    <SearchField value={query} onChangeText={setQuery} placeholder="Buscar nome, matrícula, setor ou cargo" />
    {filtered.length ? filtered.map(emp => <Pressable accessibilityRole="button" accessibilityLabel={`Abrir ficha de ${emp.name}, matrícula ${emp.registration}`} key={emp.id} onPress={() => router.push(`/person/${emp.id}` as never)}>
    ...
    </Pressable>) : <EmptyState title="Nenhum colaborador encontrado" body="Ajuste a busca para ver outros registros." />}
  </Screen>;
}
